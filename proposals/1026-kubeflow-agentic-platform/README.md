...
#### Story 2: End-to-End ML Lifecycle via A2A

An orchestrator agent (LangGraph) receives "fine-tune this model, register the best checkpoint, and deploy it." It would discover Kubeflow's Agent Card, delegate the task via A2A. The Kubeflow platform agent would internally coordinate: Trainer for fine-tuning, Katib for hyperparameter search, Hub for model registration. The orchestrator receives a single completion with the registered model URI and deployment status.
...
spec:
  mcp:
    targets:
      - name: kubeflow
        static:
          host: kubeflow-mcp.kubeflow.svc.cluster.local
          port: 8080
          protocol: StreamableHTTP
      - name: spark-history
        static:
          host: spark-history-mcp.kubeflow.svc.cluster.local
          port: 8080
          protocol: StreamableHTTP
      - name: docs
        static:
          host: kubeflow-docs-agent.kubeflow.svc.cluster.local
          port: 8080
          protocol: SSE
```

Tool namespacing: `{target_name}_{tool}` (e.g., `kubeflow_fine_tune`, `spark-history_investigate_failure`).

Policy enforcement via `AgentgatewayPolicy` (CEL-based MCP authorization):
  ...
  backend:
    mcp:
      authorization:
        ...
        policy:
          matchExpressions:
            - 'mcp.tool.target == "kubeflow" && !mcp.tool.name.contains("delete")'
            - 'mcp.tool.target == "spark-history"'
            - 'mcp.tool.target == "kubeflow" && mcp.tool.name == "get_model"'
```

### Component 3: A2A Agent Card and Delegation — [#183](https://github.com/kubeflow/mcp-server/issues/183)

The `kubeflow-mcp` server would publish a single A2A Agent Card at `/.well-known/agent-card.json` covering all its client modules (shown in v0.x format for clarity; v1.0 moves `url` into `supportedInterfaces[]`):

```json
{
  "name": "kubeflow",
  "description": "AI model training, optimization, registry, and pipelines on Kubernetes",
  ...
  "skills": [
    ...
    },
    {
      ...
      "description": "Katib-driven search over training configurations"
    },
    {
      "id": "model-registration",
      "name": "Model Registration",
      "description": "Register, version, and promote models in Model Registry"
    },
    {
      "id": "pipeline-execution",
      "name": "Pipeline Execution",
      "description": "Create, run, and monitor ML pipelines"
    }
  ],
  "securitySchemes": {
...
Companion servers (Spark History, Docs Agent) would publish their own Agent Cards independently. When federated via Agentgateway, a composite platform-level card could aggregate skills from all backends.
...
MCP prompts are user-invoked templates (slash commands) for repeatable, structured workflows. The [Spark History Server MCP](https://github.com/kubeflow/mcp-apache-spark-history-server) already ships prompts (`investigate_failure`, `compare_applications`). The Kubeflow Trainer MCP could adopt the same pattern:
...
│  ┌───────────────────────────────────────────────────────┐  │
│  │ kagent namespace                                      │  │
│  │   Agent CRD ── ModelConfig CRD ── RemoteMCPServer CRD │  │
│  └──────────────────────────┬────────────────────────────┘  │
│                             │                               │
│                             v                               │
│  ┌───────────────────────────────────────────────────────┐  │
...
spec:
  ...
  declarative:
    ...
    systemMessage: "You are a Kubeflow training assistant..."
    tools:
      - type: McpServer
        mcpServer:
          apiGroup: kagent.dev
...
| KEP-872 (Spark MCP) | Companion server federated via Agentgateway. Its prompts pattern (`investigate_failure`) could be adopted by the Trainer MCP for guided workflows |
